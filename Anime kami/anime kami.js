// ==========================================
// ⚙️ MODULE SORA — ANIME-KAMI.COM (Édition Pro - Supabase Tracker)
// ==========================================
            ...
            for (const ep of epList) {
                if (!episodeMap.has(ep.number)) {
                    ...
                    episodeMap.set(ep.number, {
                        ...
                        href: `${apiUrl}&ep=${ep.number}&lang=all`, // Le tag secret pour tout mélanger
                        number: ep.number,
                        season: 1
                    });
                }
...
// --- 4. LECTEUR (Analyse combinée VF + VOSTFR & Capture d'erreurs) ---
async function extractStreamUrl(url) {
    ...
    try {
        ...
        const streams = [];
        let extractedNames = []; 
        let failedLinks = []; // 🚨 Capture des liens morts
        ...
        for (const server of serversToProcess) {
            ...
            if (serverUrl.includes("sendvid")) {
                ...
                try {
                    // Force l'encodage windows-1251 pour le russe
                ...
                console.log(`[Lecteur] ❌ Serveur non supporté : ${serverUrl}`);
            }

            // 🚨 Si aucun flux n'a été ajouté, on enregistre l'échec pour le tableau de bord
